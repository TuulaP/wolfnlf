...
void BinarizeWolfJolion::process(cv::Mat *input2, cv::Mat *output1, char* inputname)
{

	// TODO: re-reads the img, check parameter passing 
	cv::Mat input = imread(inputname, CV_BGR2GRAY); //CV_LOAD_IMAGE_GRAYSCALE);
	...
	// ----------------------------------------------------
	cout << "Binarization version used: " << version << endl;
...
int main(int argc, char *argv[])
{
	...
	std::cerr << "===========================================================\n"
		...
		<< "===========================================================\n"
		<< "DLL version code from:  github:liris_vision/modules \n"
	    << "Added cli-handling from original (for windows) 23.12.2016 \n"
		;
		

	//usage: binarizewolfjolion [ -x <winx> -y <winy> -k <parameter> ] [ version ] <inputimage> <outputimage>
	...
	for (i = 1; i < argc; i++) {
		...
		if (*argv[i] == '/' || *argv[i] == '-') {
			...
			//cout << "Args divided: " << key; // " and " << value << endl;
			...
			case 'y':
				//cout << " y set to " << myint << endl;
				winy = myint;
				break;

			case 'k':
				//cout << " k set to " << myint << endl;
				optK = myint;
				didSpecifyK = true;
				break;
			...
			// binarisation algorithm and images
			if (argv[i] != NULL) {
				
				if (strlen(argv[i]) == 1) {
					verstr = argv[i];
					//cout << "Bin.zn version selected: " << verstr << endl;
				}
				else {
					if (inputname == NULL) {
						inputname = argv[i];
	...
	if (outputname == NULL) {
		outputname = "testi.pgm";
	}

	if (inputname == NULL)
	{
		usage(*argv);
		std::cerr << "\nInputfile required for processing !\n\n";
		exit(1);

	}

	//cerr << " infile: " << inputname << " outfile:  " << outputname << endl;

	BinarizeWolfJolion::NiblackVersion versionCode;
	

	switch (verstr[0])
	{
		case 'n':
			versionCode = BinarizeWolfJolion::NIBLACK;
			std::cerr << "Niblack (1986)\n";
			break;

		case 's':
			versionCode = BinarizeWolfJolion::SAUVOLA;
			std::cerr << "Sauvola et al. (1997)\n";
			break;

		case 'w':
			versionCode = BinarizeWolfJolion::WOLFJOLION;
			std::cerr << "Wolf and Jolion (2001)\n";
			break;

		default:
			usage(*argv);
			std::cerr << "\nInvalid version: '" << verstr[0] << "'!";
	}
	...
	if ((input.rows <= 0) || (input.cols <= 0)) {
		std::cerr << "*** ERROR: Couldn't read input image " << inputname << endl;
		exit(1);
	}
	...
	// Treat the window size
	if (winx == 0 || winy == 0) {
		std::cerr << "Input size: " << input.cols << "x" << input.rows << endl;
	...
	// Threshold
	cv::Mat outf = cv::Mat(input.rows, input.cols, CV_8U);
	//cv::Mat output(input.rows, input.cols, CV_8U);

	//NiblackSauvolaWolfJolion(input, output, versionCode, winx, winy, optK, 128, &output);

	BinarizeWolfJolion bwj = BinarizeWolfJolion(winx, winy, optK, versionCode);
	
	bwj.process(&input, &outf, inputname);
	
	std::cerr << "Writing binarized image to file '" << outputname << "'.\n";
	cv::imwrite(outputname, outf);
